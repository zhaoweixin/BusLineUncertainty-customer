<template>
  <Card class="card-overView">
    ...
    <div id="overView">
      ...
      <div id="newpanel">
        <div id="overviewtooltip_box">
        </div>
        <div id="overviewtooltip_form">
          <form>
            <!-- <label style="position: relative;float: left; width: 5%"
              ><b>Car_id:</b></label
            > -->
            ...
            <Input
              v-model="value1"
              placeholder="car no."
              style="position: relative; float:left; width: 49%; padding-left: 5%; padding-top:1.5%"
            />

            <!-- <label style="position: relative; float: left; width: 5%"
              ><b>Start Station:</b></label
            > -->
            <Input
              v-model="value2"
            ...
            <!-- <label style="position: relative; tfloat: left; width: 5%"
            ...
            <button
              @click="Commite"
              style="position: relative; float:left; width: 20%; left: 5%; top:8px; border: none; border-radius: 7px; font-weight: 600"
            >
              Input
            </button>
            ...
            <input
              id="output"
              type="text"
              value="show arrival time"
              style="position: relative; float:left; width: 49%; left: 8%; top:8px; border:none; font-weight: 600;"
            />
  ...
  methods: {
    initToolBox(){
      ...
      let innerWidth = toolboxWidth - margin.left - margin.right,
          tooltipHeight = toolboxHeight * 0.05,
          tooltipSubHeight = toolboxHeight * 0.1
        // Add title to graph
    ...
    Commite() {
      ...
      this.$axios.get("http://localhost:8080/static/PPF.json").then((data) => {
        ...
        function PD(station, date, array) {
          ...
          if (
            new Date("2000/01/01 " + date).getTime() <=
            new Date("2000/01/01 " + want["" + mid][2]).getTime()
          ) {
            now = want["" + mid][2];
          } else if (
            new Date("2000/01/01 " + date).getTime() >
              new Date("2000/01/01 " + want["" + mid][2]).getTime() &&
            new Date("2000/01/01 " + date).getTime() <=
              new Date("2000/01/01 " + want["" + mid][4]).getTime()
          ) {
            now = want["" + mid][4];
          } else {
            mid = mid + 1;
        ...
        var xScale = d3
          .scaleTime()
          .domain([
            d3.timeParse("%H:%M:%S")(this.value4),
            d3.timeParse("%H:%M:%S")(array[this.value3][index][4]),
          ])
          .range([40, width - margin.right]);
        var yScale = d3
          .scaleLinear()
          .domain([parseFloat(this.value2) - 1, parseFloat(this.value3) + 2])
          .range([innerHeight, 0]);
        console.log(
          "domain:",
          parseFloat(this.value2) - 1,
          "---",
          parseFloat(this.value3) + 2
        );
        ...
        //后面的点都取第三个值 为[2]
        for (
          let i = parseFloat(this.value2) + 1;
          i <= parseFloat(this.value3);
          i++
        ) {
        ...
        g.append("g")
          ...
          .enter()
          .append("circle")
          .attr("cx", (d) => xScale(d3.timeParse("%H:%M:%S")(d[1])))
        ...
        var line = d3
          .line()
          .x((d) => xScale(d3.timeParse("%H:%M:%S")(d.time)))
        ...
        console.log(
          "res",
          Object.keys(array).filter((d) => {
            return (
              parseInt(d) >= parseInt(this.value2) &&
              parseInt(d) <= parseInt(this.value3)
            );
          })
        );
        ...
        for (let key in array) {
          ...
          //如果是目标站点内
          if (
            parseInt(this.value2) <= parseInt(key) &&
            parseInt(key) <= parseInt(this.value3)
          ) {
